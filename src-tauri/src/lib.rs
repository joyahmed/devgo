...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        // ⭐ every command is logged by name here rather than by a line
        // added to each command body: `generate_handler!` expands to a
        // plain `Fn(Invoke) -> bool`, so wrapping it is tauri v2's global
        // hook on the command boundary, and one wrapper cannot be
        // forgotten the way a hundred call sites can. Logging on this side
        // of the IPC and not in the frontend is deliberate: a frontend log
        // proves the frontend tried, and the question is whether the
        // command ran.
        //
        // ⛔ the name only. See `runtime_log::INVOKE_PREFIX` for why no
        // argument ever joins it.
        .invoke_handler(move |invoke: tauri::ipc::Invoke<tauri::Wry>| {
            runtime_log::invoked(invoke.message.command());
            dispatch(invoke)
        })
        .build(context)
...
/// The command table. Split out of `run` only so the invoke-logging
/// wrapper above stays one readable line instead of being buried under a
/// hundred command names.
fn command_dispatch(
