...
Every chapter is a branch: check NN.name out and the tree builds and runs exactly as the chapter leaves it, with that chapter's text at `docs/chapters/NN-name.md` beside it. The text lives with the tree it describes; `main` carries all of it, plus [00 — Prerequisites](./00-prerequisites.md) and the [Rust](./appendix/rust-concepts.md) and [Tauri](./appendix/tauri-concepts.md) appendices.
